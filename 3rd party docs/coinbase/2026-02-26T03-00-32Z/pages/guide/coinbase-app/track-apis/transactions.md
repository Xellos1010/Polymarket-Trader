...
The Send money API is asynchronous, which means that Coinbase may delay or cancel the send when necessary. Coinbase recommends that you poll the `status` field for the `completed` state with the [Show a Transaction](#show-a-transaction) API. You can also see if a transaction is `pending` in the Send Money API response. When used with OAuth2 authentication, this endpoint requires [two factor authentication](https://developer.chrome.com/coinbase-app/oauth2-integration/2fa).
