...
The Send money API is asynchronous, which means that Coinbase may delay or cancel the send when necessary. Coinbase recommends that you poll the `status` field for the `completed` state with the [Show a Transaction API](https://developer.chrome.com/coinbase-app/track-apis/transactions). You can also see if a transaction is `pending` in the Send Money API response. When used with OAuth2 authentication, this endpoint requires [two factor authentication](https://developer.chrome.com/coinbase-app/authentication-authorization/oauth2/2fa).
...
**\[Recommended\]** A UUIDv4 token to ensure [idempotence](https://en.wikipedia.org/wiki/Idempotence).
...
For select currencies, `destination_tag` or `memo` indicates the beneficiary or destination
...
Network to be sent on for a blockchain address send. Defaults to the currency’s default network.
...
Visit the [Guide](https://developers.coinbase.com/docs/wallet/guides/travel-rule) for more details
