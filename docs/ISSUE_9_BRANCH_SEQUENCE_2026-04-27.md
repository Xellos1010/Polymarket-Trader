...
## Acceptance criteria
- Only one code-bearing helper-stack base is used for issue `#9`.
- The next PR after `#37` is runtime wiring only, not another planning branch.
- Queue persistence remains limited to `draft` and `cancel_requested`.
...
- Do not treat tracker/docs work as evidence that Phase 1 ROI gates have passed.
- Do not widen the queue scope beyond restart-safe operator review.
- Do not merge overlapping helper-stack branches without resolving sequencing first.
