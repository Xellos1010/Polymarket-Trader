...
- The materially relevant merged queue is now:
...
- The open draft PR stack is `#32` through `#38`.
- PR `#37` is the only open code-bearing approval-queue branch worth advancing as the next engineering base.
- PR `#32` overlaps the same helper-stack surface and is the main merge/conflict risk.
- PRs `#33`, `#34`, `#35`, `#36`, and `#38` are docs/control-tower drafts and should not drive implementation sequencing.
- The next concrete runtime blocker is still issue `#9`: final `crates/pt-cli/src/coinbase.rs` wiring for restart-safe approval-queue hydration and reconciliation.
- No fresh local cargo validation was run in this environment, so local gate status remains unknown here.

## In Progress

- Keep PR `#37` as the canonical helper-stack base for issue `#9`.
- Start the next narrow runtime slice after that helper stack settles:
  - open `ApprovalQueueStore` from `storage.sqlite_path`
  - hydrate `draft` and `cancel_requested` queue rows on startup
  - reconcile queue state after local order lifecycle mutations
  - reconcile queue state after live-order sync and identity changes
- Keep `/api/v1/orders` and `/api/v1/approval-queue` read-only.
- Do not widen scope into live enablement, credential handling, or risk-cap changes.

## Next Queue

1. Validate and land PR `#37` as the canonical issue `#9` helper-stack base.
2. Open one narrow follow-up PR for `crates/pt-cli/src/coinbase.rs` runtime wiring only.
3. Run the local-first validation ladder in a Rust-enabled environment:
   - `cargo fmt --all`
...
4. Retire or supersede overlapping planning drafts once the code-bearing sequence is clear.
5. Resume Phase 1 replay/paper repeatability evidence only after the queue-persistence blocker is settled.
